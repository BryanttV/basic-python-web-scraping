...
import requests
import lxml.html as html # Converts a HTML file to XPath file
...
def parse_notice(link: str, today: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                title : str = parsed.xpath(XPATH_TITLE)[0]
                title = re.sub(r'[^\w ]', '', title)
                summary : str = parsed.xpath(XPATH_SUMMARY)[0]
                body : str = parsed.xpath(XPATH_BODY)
            except IndexError:
                return
